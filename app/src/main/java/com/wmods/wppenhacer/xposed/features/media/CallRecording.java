    ...
    private boolean shouldRecord(String phoneNumber) {
        try {
            ...
            if (mode == 2) { // Exclude Selected (Blacklist)
                // If phone number not yet known, allow (not blacklisted).
                // If whitelist is empty, record everyone.
                if (phoneNumber == null)
                    return true;
                String cleanPhone = phoneNumber.replaceAll("[^0-9]", "");
                return !isNumberInList(cleanPhone, blacklist);
            } else if (mode == 3) { // Include Selected (Whitelist)
                // If whitelist is empty, record nobody.
                if (whitelist.equals("[]") || whitelist.isEmpty())
                    return false;
                // If phone number not yet identified, deny (not confirmed in whitelist).
                if (phoneNumber == null)
                    return false;
                String cleanPhone = phoneNumber.replaceAll("[^0-9]", "");
                return isNumberInList(cleanPhone, whitelist);
            } else if (mode == 1) { // Record Unknown Only (contacts not in address book)
                // Without a contact lookup we conservatively allow recording when number
                // is unknown / not yet resolved, and allow if number could not be matched.
                return true;
            }
        } catch (Exception e) {
